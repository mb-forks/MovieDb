...
namespace MovieDb
{
            ...
            IHasOrder
    {
        ...
        public async Task<IEnumerable<RemoteSearchResult>> GetSearchResults(EpisodeInfo searchInfo, CancellationToken cancellationToken)
        {
            ...
            var metadataResult = await this.GetMetadata(searchInfo, cancellationToken).ConfigureAwait(false);
        ...
        public async Task<MetadataResult<Episode>> GetMetadata(EpisodeInfo info, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var response = await GetEpisodeInfo(seriesTmdbId, seasonNumber.Value, episodeNumber.Value, info.MetadataLanguage, info.MetadataCountryCode, cancellationToken).ConfigureAwait(false);
                ...
                if (credits != null)
                {
                    ...
                    if (credits.cast != null)
                    {
                        ...
                        {
                            result.AddPerson(new PersonInfo { Name = actor.name.Trim(), Role = actor.character, Type = PersonType.Actor });
                        }
                    }
                    ...
                    if (credits.guest_stars != null)
                    {
                        ...
                        {
                            result.AddPerson(new PersonInfo { Name = guest.name.Trim(), Role = guest.character, Type = PersonType.GuestStar });
                        }
                    }
                    ...
                    if (credits.crew != null)
                    {
                        ...
                        {
                            // Normalize this
                            PersonType type = PersonType.Lyricist;
                            ...
                            PersonType parsedType;
                            if (Enum.TryParse(department, true, out parsedType))
                            {
                                type = parsedType;
                            }
                            else if (Enum.TryParse(person.job, true, out parsedType))
                            {
                                type = parsedType;
                            }
